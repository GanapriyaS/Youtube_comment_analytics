...
def youtube_search_keyword(category):
	...
	data = pd.read_sql(f'''SELECT * from public.videos where category='{category}' ''', con=conn)
	final_return=[]

	for index, row in data.iterrows():
		counts=[row["extremely_happy"],row["happy"],row["neutral"],row["sad"],row["angry"]]
		total=np.sum(counts)
		counts=counts/total
...
def search_results():
	...
	if request.method=="GET":
		...
		result=youtube_search_keyword('GCT')
		print(result)
		return render_template('index.html',result=result)
